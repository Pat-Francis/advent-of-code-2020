# --- Part Two ---
# The Elves in accounting are thankful for your help; one of them even offers you a starfish coin they had left over from a past vacation. They offer you a second one if you can find three numbers in your expensesense report that meet the same criteria.
# Using the above example again, the three entries that sum to 2020 are 979, 366, and 675. Multiplying them together produces the answer, 241861950.
# In your expensesense report, what is the product of the three entries that sum to 2020?
...
t = 2020
arr_size = len(expenses)
def find3nums(expenses, arr_size, t):
    expenses.sort()
    for i in range(0,len(expenses) - 2):
        s = i + 1
        e = len(expenses) - 1
        while (s < e):
            if(expenses[i] + expenses[s] + expenses[e] == t):
                print(expenses[i],', ',expenses[s],', ',expenses[e], 'product is ', expenses[i] * expenses[s] * expenses[e])
                return True
            elif(expenses[i] + expenses[s] + expenses[e] < t):
                s += 1
            else:
                e -= 1
    return False

find3nums(expenses, arr_size, t)